...
help() {
    ...
    echo "  ghrao [org] [repo] - View GitHub Actions runs (auto-detects current repo)"
    echo "  ghraw [org] [repo] - Interactive search & watch/view GitHub Actions logs"
    echo "  ghrag [org] [repo] - Interactive search & copy workflow IDs to clipboard"
    echo "  ghrag -w      - Interactive search & copy job IDs to clipboard"
    ...
    echo "💡 Tips:"
    echo "  • GitHub commands auto-detect current repo or prompt for org/repo selection"
    echo "  • All GitHub functions use fuzzy search (fzf) and never error out"
    echo "  • Functions automatically fetch organizations from GitHub API if needed"
